...
function formatTodo(row, db) {
    const children = db.prepare('SELECT id FROM todos WHERE parent_id = ? ORDER BY position ASC, created_at ASC').all(row.id).map(r => r.id);
    const agentIds = db.prepare('SELECT id FROM agents WHERE todo_id = ? ORDER BY created_at ASC').all(row.id).map(r => r.id);
    return {
        id: row.id,
        date: row.date,
        title: row.title,
...
function buildTodoJsonData(todoId, db) {
    const row = db.prepare('SELECT * FROM todos WHERE id = ?').get(todoId);
    if (!row)
        return null;
    const subRows = db.prepare('SELECT * FROM todos WHERE parent_id = ? ORDER BY position ASC').all(todoId);
    return {
        id: row.id,
        title: row.title,
        note: row.note,
...
export async function todosRoutes(app) {
    ...
    app.get('/api/todos', async (req, reply) => {
        ...
        const db = getDb();
        const rows = db.prepare('SELECT * FROM todos WHERE date = ? AND parent_id IS NULL ORDER BY position ASC, created_at ASC').all(date);
    ...
    `).run(id, body.date, body.title, body.note, body.parentId, now, now);
        ...
        if (jsonData)
            await writeTodoJson(body.date, id, jsonData);
        // Update parent's .todo.json if applicable
        if (body.parentId) {
            const parent = db.prepare('SELECT * FROM todos WHERE id = ?').get(body.parentId);
            if (parent) {
    ...
    app.patch('/api/todos/:id', async (req, reply) => {
        ...
        if (body.title !== undefined)
            db.prepare('UPDATE todos SET title = ?, updated_at = ? WHERE id = ?').run(body.title, now, id);
        if (body.note !== undefined)
            db.prepare('UPDATE todos SET note = ?, updated_at = ? WHERE id = ?').run(body.note, now, id);
