...
interface ActiveProcess {
  ...
  // Set to true when intentionally killed so handleExit skips status updates
  killed: boolean;
  // Model context window size (tokens); updated from event stream when available
...
function getDefaultContextWindow(model: string): number {
  if (/claude/.test(model)) return 200000;
  if (/o[1-4][-_]|^o[1-4]$/.test(model)) return 200000;
...
function updateAgentStatus(agentId: string, status: string, extra?: { ctxPct?: number; toolCalls?: number; sessionId?: string; endedAt?: number; tokensUsed?: number; tokensTotal?: number }): void {
  const db = getDb();
